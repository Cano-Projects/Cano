...
void replace(Buffer *buffer, State *state, char *new_str, size_t old_str_s,
             size_t new_str_s) {
...
void find_and_replace(Buffer *buffer, State *state, char *old_str,
                      char *new_str) {
...
void motion_g(State *state) {
    ...
    state->buffer->cursor =
        state->buffer->rows.data[state->repeating.repeating_count - 1].start;
    state->repeating.repeating_count = 0;
    if (state->leader != LEADER_D)
        return;
...
void motion_G(State *state) {
    ...
    if (state->repeating.repeating_count > 0) {
        if (state->repeating.repeating_count >= state->buffer->rows.count)
            state->repeating.repeating_count = state->buffer->rows.count;
        state->buffer->cursor =
            state->buffer->rows.data[state->repeating.repeating_count - 1]
                .start;
        state->repeating.repeating_count = 0;
    } else {
...
void motion_b(State *state) {
    ...
    if (buffer->cursor - 1 > 0 &&
        !isword(buffer->data.data[buffer->cursor - 1]))
        buffer->cursor--;
    while (buffer->cursor - 1 > 0 &&
           (isword(buffer->data.data[buffer->cursor - 1]) ||
            isspace(buffer->data.data[buffer->cursor + 1]))) {
        buffer->cursor--;
    }
    if (buffer->cursor - 1 == 0)
        buffer->cursor--;
...
void motion_w(State *state) {
    ...
    while (buffer->cursor < buffer->data.count &&
           (isword(buffer->data.data[buffer->cursor]) ||
            isspace(buffer->data.data[buffer->cursor + 1]))) {
        buffer->cursor++;
    }
...
int handle_motion_keys(Buffer *buffer, State *state, int ch,
                       size_t *repeating_count) {
    (void)repeating_count;
    switch (ch) {
    case 'g': { // Move to the start of the file or to the line specified by
                // repeating_count
        motion_g(state);
    } break;
    case 'G': { // Move to the end of the file or to the line specified by
                // repeating_count
        motion_G(state);
    } break;
...
int handle_normal_to_insert_keys(Buffer *buffer, State *state) {
    ...
    case 'I': {
        ...
        while (buffer->cursor < cur.end &&
               isspace(buffer->data.data[buffer->cursor]))
            buffer->cursor++;
        state->config.mode = INSERT;
    } break;
    case 'a':
...
void buffer_handle_undo(State *state, Undo *undo) {
    ...
    case INSERT_CHARS:
        state->cur_undo.type =
            (undo->data.count > 1) ? DELETE_MULT_CHAR : DELETE_CHAR;
...
void handle_normal_keys(Buffer *buffer, Buffer **modify_buffer, State *state) {
    ...
    default: {
        if (state->is_exploring)
            handle_explore_keys(state);
        if (handle_modifying_keys(buffer, state))
            break;
        if (handle_motion_keys(buffer, state, state->ch,
                               &state->repeating.repeating_count))
            break;
        if (handle_normal_to_insert_keys(buffer, state))
            break;
    } break;
    }
    if (state->repeating.repeating_count == 0) {
...
void handle_insert_keys(Buffer *buffer, Buffer **modify_buffer, State *state) {
    ...
    default: { // Handle other characters
        ...
        Brace cur_brace = {0};
        if (buffer->cursor == 0)
            cur_brace = find_opposite_brace(buffer->data.data[0]);
        else
...
void handle_command_keys(Buffer *buffer, Buffer **modify_buffer, State *state) {
    ...
    case ENTER: {
        if (state->command[0] == '!') {
            ...
            while (fgets(state->status_bar_msg, sizeof(state->status_bar_msg),
                         file) != NULL) {
            ...
            size_t command_s = 0;
            Command_Token *command = lex_command(
                state, view_create(state->command, state->command_s),
                &command_s);
...
void handle_visual_keys(Buffer *buffer, Buffer **modify_buffer, State *state) {
    ...
    default: {
        handle_motion_keys(buffer, state, state->ch,
                           &state->repeating.repeating_count);
        if (buffer->visual.is_line) {
            buffer->visual.end = buffer->rows.data[buffer_get_row(buffer)].end;
            if (buffer->visual.start >= buffer->visual.end) {
                buffer->visual.end =
                    buffer->rows.data[buffer_get_row(buffer)].start;
                buffer->visual.start =
                    buffer->rows
                        .data[index_get_row(buffer, buffer->visual.start)]
                        .end;
            }
        } else {
            buffer->visual.end = buffer->cursor;
