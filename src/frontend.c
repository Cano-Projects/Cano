...
size_t num_of_open_braces(Buffer *buffer) {
    ...
    while (index > 0) {
        index--;
        if (buffer->data.data[index] == '"' || buffer->data.data[index] == '\'')
            in_str = !in_str;
...
void state_render(State *state) {
    ...
    if (state->is_exploring)
        col = 0;
    if (cur_row <= row_render_start)
        row_render_start = cur_row;
    if (cur_row >= row_render_start + state->main_row)
        row_render_start = cur_row - state->main_row + 1;
    if (col <= col_render_start)
        col_render_start = col;
    if (col >= col_render_start + state->main_col)
        col_render_start = col - state->main_col + 1;
    ...
    mvwprintw(state->status_bar, 0, state->gcol / 2, "%zu:%zu", cur_row + 1,
              col + 1);
    mvwprintw(state->status_bar, 0, state->main_col - 11, "%c",
              state->config.leaders[state->leader]);
    mvwprintw(state->status_bar, 0, 0, "%.7s",
              string_modes[state->config.mode]);
    mvwprintw(state->status_bar, 0, state->main_col - 5, "%.*s",
              (int)state->num.count, state->num.data);

    if (state->config.mode == COMMAND || state->config.mode == SEARCH)
        mvwprintw(state->status_bar, 1, 0, ":%.*s", (int)state->command_s,
                  state->command);
    if (state->is_exploring) {
        wattron(state->main_win, COLOR_PAIR(BLUE_COLOR));
        for (size_t i = row_render_start;
             i <= row_render_start + state->main_row; i++) {
            if (i >= state->files->count)
                break;
            // TODO: All directories should be displayed first, afterwards files
            size_t print_index_y = i - row_render_start;
            mvwprintw(state->main_win, print_index_y, 0, "%s",
                      state->files->data[i].name);
        }
    } else {
        for (size_t i = row_render_start;
             i <= row_render_start + state->main_row; i++) {
            if (i >= state->buffer->rows.count)
                break;
            size_t print_index_y = i - row_render_start;
            wattron(state->line_num_win, COLOR_PAIR(YELLOW_COLOR));
            if (state->config.relative_nums) {
                if (cur_row == i) {
                    mvwprintw(state->line_num_win, print_index_y, 0, "%zu",
                              i + 1);
                } else {
                    mvwprintw(state->line_num_win, print_index_y, 0, "%zu",
                              (size_t)abs((int)i - (int)cur_row));
                }
            } else {
            ...
            if (state->config.syntax) {
                token_s = generate_tokens(state->buffer->data.data +
                                              state->buffer->rows.data[i].start,
                                          state->buffer->rows.data[i].end -
                                              state->buffer->rows.data[i].start,
                                          token_arr, &token_capacity);
            }
            Color_Pairs color = 0;
            for (size_t j = state->buffer->rows.data[i].start;
                 j < state->buffer->rows.data[i].end; j++) {
                if (j < state->buffer->rows.data[i].start + col_render_start ||
                    j > state->buffer->rows.data[i].end + col + state->main_col)
                    continue;
                size_t col = j - state->buffer->rows.data[i].start;
                size_t print_index_x = col - col_render_start;

                for (size_t chr = state->buffer->rows.data[i].start; chr < j;
                     chr++) {
                    if (state->buffer->data.data[chr] == '\t')
                        print_index_x += 3;
                }
                size_t keyword_size = 0;
                if (state->config.syntax &&
                    is_in_tokens_index(token_arr, token_s, col, &keyword_size,
                                       &color)) {
                    wattron(state->main_win, COLOR_PAIR(color));
                    off_at = col + keyword_size;
                }
                if (col == off_at)
                    wattroff(state->main_win, COLOR_PAIR(color));
                if (col > state->buffer->rows.data[i].end)
                    break;
                int between =
                    (state->buffer->visual.start > state->buffer->visual.end)
                        ? is_between(state->buffer->visual.end,
                                     state->buffer->visual.start,
                                     state->buffer->rows.data[i].start + col)
                        : is_between(state->buffer->visual.start,
                                     state->buffer->visual.end,
                                     state->buffer->rows.data[i].start + col);
                if (state->config.mode == VISUAL && between)
                    wattron(state->main_win, A_STANDOUT);
                else
                    wattroff(state->main_win, A_STANDOUT);
                mvwprintw(state->main_win, print_index_y, print_index_x, "%c",
                          state->buffer->data
                              .data[state->buffer->rows.data[i].start + col]);
            }
            free(token_arr);
        }
    }
    ...
    if (state->config.mode == COMMAND || state->config.mode == SEARCH) {
        ...
        frontend_move_cursor(state->main_win, cur_row - row_render_start,
                             col - col_render_start);
    }
}
...
void frontend_init(State *state) {
    ...
    WINDOW *main_win = newwin(state->grow - 2, state->gcol - line_num_width, 0,
                              line_num_width);
...
void frontend_cursor_visible(int value) { curs_set(value); }

int frontend_getch(WINDOW *window) { return wgetch(window); }

void frontend_resize_window(State *state) {
    getmaxyx(stdscr, state->grow, state->gcol);
    wresize(state->main_win, state->grow - 2,
            state->gcol - state->line_num_col);
