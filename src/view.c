    ...
    for (i = 0; i < view.len; i++) {
        if (view.data[i] == c) {
            if (arr_index < arr_s - 2) {
                ...
                arr[arr_index++] =
                    view_create(view.data + i + 1, view.len - i - 1);
                return arr_index;
            }
        }
    }
...
int view_to_int(String_View view) { return strtol(view.data, NULL, 10); }

float view_to_float(String_View view) { return strtof(view.data, NULL); }
