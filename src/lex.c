...
int is_in_tokens_index(Token *token_arr, size_t token_s, size_t index,
                       size_t *size, Color_Pairs *color) {
    ...
    while (view.len > 0) {
        if (isalpha(view.data[0])) {
            ...
            if (token_arr_s >= *token_arr_capacity) {
                token_arr = realloc(token_arr,
                                    sizeof(Token) * (*token_arr_capacity) * 2);
            ...
            token_arr[token_arr_s++] = token;
        } else if (view.len >= 2 && view.data[0] == '/' &&
                   view.data[1] == '/') {
            Token token = {
                .type = Type_Comment,
                .index = view.data - line,
