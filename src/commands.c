...
static char *tt_string[TT_COUNT] = {
    ...
    "mult",    "div",         "echo",    "w",          "e",    "we",
    "ident",   "special key", "string",  "config var", "int",  "float"};

static Ctrl_Key ctrl_keys[] = {
    {"<ctrl-a>", ctrl('a')}, {"<ctrl-b>", ctrl('b')}, {"<ctrl-c>", ctrl('c')},
    {"<ctrl-d>", ctrl('d')}, {"<ctrl-e>", ctrl('e')}, {"<ctrl-f>", ctrl('f')},
    {"<ctrl-g>", ctrl('g')}, {"<ctrl-h>", ctrl('h')}, {"<ctrl-i>", ctrl('i')},
    {"<ctrl-j>", ctrl('j')}, {"<ctrl-k>", ctrl('k')}, {"<ctrl-l>", ctrl('l')},
    {"<ctrl-m>", ctrl('m')}, {"<ctrl-n>", ctrl('n')}, {"<ctrl-o>", ctrl('o')},
    {"<ctrl-p>", ctrl('p')}, {"<ctrl-q>", ctrl('q')}, {"<ctrl-r>", ctrl('r')},
    {"<ctrl-s>", ctrl('s')}, {"<ctrl-t>", ctrl('t')}, {"<ctrl-u>", ctrl('u')},
    {"<ctrl-v>", ctrl('v')}, {"<ctrl-w>", ctrl('w')}, {"<ctrl-x>", ctrl('x')},
    {"<ctrl-y>", ctrl('y')}, {"<ctrl-z>", ctrl('z')},
};
...
Command_Token create_token(State *state, String_View command) {
    ...
    String_View result = {.data = starting.data,
...
void print_token(Command_Token token) {
    printf("location: %zu, type: %d, value: " View_Print "\n", token.location,
           token.type, View_Arg(token.value));
}

int expect_token(State *state, Command_Token token, Command_Type type) {
    if (token.type != type) {
        sprintf(state->status_bar_msg, "Invalid arg, expected %s but found %s",
                tt_string[type], tt_string[token.type]);
...
int get_special_char(String_View view) {
    if (view_cmp(view, LITERAL_CREATE("<space>"))) {
        ...
        for (size_t i = 0; i < NUM_OF_CTRL_KEYS; i++) {
            if (view_cmp(view, view_create(ctrl_keys[i].name,
                                           strlen(ctrl_keys[i].name)))) {
...
Bin_Expr *parse_bin_expr(State *state, Command_Token *command,
    ...
    if (expr->operator== OP_NONE)
        return NULL;
    if (!expect_token(state, command[2], TT_INT_LIT))
        return NULL;
...
Node *parse_command(State *state, Command_Token *command, size_t command_s) {
    ...
    case TT_SET_VAR:
        ...
        if (!expect_token(state, command[1], TT_CONFIG_IDENT) ||
            !expect_token(state, command[2], TT_INT_LIT))
            return NULL;
    ...
    case TT_SET_OUTPUT:
        ...
        val.as_str =
            (Str_Literal){.value = view_string_internals(command[1].value)};
    ...
    case TT_SET_MAP:
        ...
        if (!expect_token(state, command[2], TT_STRING))
            return NULL;
    ...
    case TT_LET:
        ...
        if (!expect_token(state, command[1], TT_IDENT) ||
            !expect_token(state, command[2], TT_INT_LIT))
            return NULL;
        val.as_ident = (Identifier){.name = command[1].value};
        root->left = create_node(NODE_IDENT, val);
    ...
    case TT_ECHO:
        ...
        if (!expect_token(state, command[1], TT_IDENT) &&
            !expect_token(state, command[1], TT_STRING))
            return NULL;
...
void interpret_command(Buffer *buffer, State *state, Node *root) {
    ...
    case NODE_KEYWORD:
        ...
        case TT_SET_MAP: {
            ...
            Map map = {.b = str,
                       .b_s = root->right->value.as_str.value.len + 1};
        ...
        case TT_ECHO:
            ...
            if (root->right->type == NODE_STR) {
                ...
                for (size_t i = 0; i < state->variables.count; i++) {
                    String_View var =
                        view_create(state->variables.data[i].name,
                                    strlen(state->variables.data[i].name));
                    if (view_cmp(root->right->value.as_ident.name, var)) {
                        if (state->variables.data[i].type == VAR_INT)
                            sprintf(state->status_bar_msg, "%d",
                                    state->variables.data[i].value.as_int);
                        else if (state->variables.data[i].type == VAR_FLOAT)
                            sprintf(state->status_bar_msg, "%f",
                                    state->variables.data[i].value.as_float);
                        return;
                    }
                }
            }
