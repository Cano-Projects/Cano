...
void buffer_calculate_rows(Buffer *buffer) {
    ...
    DA_APPEND(&buffer->rows,
              ((Row){.start = start, .end = buffer->data.count}));
}
...
void buffer_insert_char(State *state, Buffer *buffer, char ch) {
    ...
    memmove(&buffer->data.data[buffer->cursor + 1],
            &buffer->data.data[buffer->cursor],
            buffer->data.count - 1 - buffer->cursor);
...
void buffer_delete_char(Buffer *buffer, State *state) {
    ...
    if (buffer->cursor < buffer->data.count) {
        ...
        memmove(&buffer->data.data[buffer->cursor],
                &buffer->data.data[buffer->cursor + 1],
                buffer->data.count - buffer->cursor - 1);
        buffer->data.count--;
        buffer_calculate_rows(buffer);
    }
}
...
size_t buffer_get_row(const Buffer *buffer) {
    ...
    for (size_t i = 0; i < buffer->rows.count; i++) {
        if (buffer->rows.data[i].start <= buffer->cursor &&
...
size_t index_get_row(Buffer *buffer, size_t index) {
    ...
    ASSERT(buffer->rows.count >= 1, "there must be at least one line");
    for (size_t i = 0; i < buffer->rows.count; i++) {
        if (buffer->rows.data[i].start <= index &&
            index <= buffer->rows.data[i].end) {
            return i;
        }
    }
    return 0;
...
void buffer_yank_line(Buffer *buffer, State *state, size_t offset) {
    size_t row = buffer_get_row(buffer);
    // check boundaries
    if (offset > index_get_row(buffer, buffer->data.count))
        return;
    ...
    state->clipboard.str = realloc(
        state->clipboard.str, initial_s + state->clipboard.len * sizeof(char));
    if (row > 0)
        line_offset = -1;
    else {
        state->clipboard.len--;
    ...
    strncpy(state->clipboard.str + initial_s,
            buffer->data.data + cur.start + line_offset, state->clipboard.len);
...
void buffer_yank_char(Buffer *buffer, State *state) {
    ...
    // resize the clipboard as necessary
    state->clipboard.str =
        realloc(state->clipboard.str, state->clipboard.len * sizeof(char));
    ASSERT(state->clipboard.str != NULL, "clipboard was null");
    strncpy(state->clipboard.str, buffer->data.data + buffer->cursor,
            state->clipboard.len);
}
...
void buffer_yank_selection(Buffer *buffer, State *state, size_t start,
    ...
    state->clipboard.str =
        realloc(state->clipboard.str, state->clipboard.len * sizeof(char) + 1);
    ASSERT(state->clipboard.str != NULL, "clipboard was null %zu",
           state->clipboard.len);
    strncpy(state->clipboard.str, buffer->data.data + start,
            state->clipboard.len);
}
...
void buffer_insert_selection(Buffer *buffer, Data *selection, size_t start) {
    ...
    if (buffer->data.count + size >= buffer->data.capacity) {
        ...
        buffer->data.data = realloc(buffer->data.data,
                                    sizeof(char) * buffer->data.capacity + 1);
...
int skip_to_char(Buffer *buffer, int cur_pos, int direction, char c) {
    ...
    if (buffer->data.data[cur_pos] == c) {
        ...
        while (cur_pos > 0 && cur_pos <= (int)buffer->data.count &&
               buffer->data.data[cur_pos] != c) {
            if (cur_pos > 1 && cur_pos < (int)buffer->data.count &&
                buffer->data.data[cur_pos] == '\\') {
                cur_pos += direction;
            }
            cur_pos += direction;
        }
...
void buffer_next_brace(Buffer *buffer) {
    ...
    while (cur_pos >= 0 && cur_pos <= (int)buffer->data.count) {
        ...
        if (cur_brace.brace == '0')
            continue;
        if ((cur_brace.closing && direction == -1) ||
            (!cur_brace.closing && direction == 1)) {
            brace_stack++;
        } else {
            if (brace_stack-- == 0 &&
                cur_brace.brace ==
                    find_opposite_brace(initial_brace.brace).brace) {
...
void buffer_brace_indent(State *state, Brace brace) {
    ...
    if (brace.brace != '0' && brace.closing) {
        ...
        if (state->config.indent > 0) {
            for (size_t i = 0;
                 i < state->config.indent * (state->num_of_braces - 1); i++) {
                buffer_insert_char(state, buffer, ' ');
            }
            handle_move_left(state,
                             state->config.indent * (state->num_of_braces - 1));
        } else {
...
State init_state(void) {
    ...
    state.config.vars[0] =
        (Config_Vars){{"syntax", sizeof("syntax") - 1}, &state.config.syntax};
    state.config.vars[1] =
        (Config_Vars){{"indent", sizeof("indent") - 1}, &state.config.indent};
    state.config.vars[2] = (Config_Vars){
        {"auto-indent", sizeof("auto-indent") - 1}, &state.config.auto_indent};
    state.config.vars[3] = (Config_Vars){{"undo-size", sizeof("undo-size") - 1},
                                         &state.config.undo_size};
    state.config.vars[4] = (Config_Vars){{"relative", sizeof("relative") - 1},
